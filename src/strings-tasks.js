...
function getStringLength(myStirng) {
  if (typeof myStirng === 'string') {
    return myStirng.length;
  }
  return 0;
}
/**
...
function isPalindrome(str) {
  const MyStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
...
function extractNameFromTemplate(value) {
  const startIndex = value.indexOf('Hello, ') + 7;
  const endIndex = value.indexOf('!', startIndex);
  const name = value.slice(startIndex, endIndex).trim();
  return name;
}
